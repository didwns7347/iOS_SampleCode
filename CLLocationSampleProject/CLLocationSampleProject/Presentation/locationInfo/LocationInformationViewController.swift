...
class LocationInformationViewController: UIViewController {
    ...
    private func bind(_ viewModel: LocationInformationViewModel) {
        detailListBackgroundView.bind(vm: viewModel.detailListBackgroundViewModel)
        ...
        viewModel.detailListCellData
            .drive(detailList.rx.items) { tv, row, data in
                let cell = tv.dequeueReusableCell(withIdentifier: DetailListCell.id, for: IndexPath(row: row, section: 0)) as! DetailListCell
    ...
    private func attribute() {
        ...
        detailList.register(DetailListCell.self, forCellReuseIdentifier: DetailListCell.id)
        ...
        return Binder(base) { base, points in
            ...
            let items = points
                ...
                .map { offset, point -> MTMapPOIItem in
                    ...
                    print(" \(mapPOIItem.mapPoint.mapPointGeo().latitude) , \( mapPOIItem.mapPoint.mapPointGeo().longitude)")
                    return mapPOIItem
                }
